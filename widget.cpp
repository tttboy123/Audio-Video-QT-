    ...
    , ui(new Ui::Widget)
{
    ...
    m_pVideoWidget = new QVideoWidget(this);//创建播放窗口
    ...
    ui->btn_open->setFocusPolicy(Qt::NoFocus);
        ...
        ui->btn_last->setFocusPolicy(Qt::NoFocus);
...
void Widget::on_btn_open_clicked()
{
    ...
    m_strlst.clear();
    m_strlst = QFileDialog::getOpenFileNames(this,"打开","C:\"",
                                                            "allfiles(*.*);;"
                                                               "MP3(*.mp3);;"
                                                               "MP4(*.mp4);;"
                                                               "AVI(*.avi)");
    ui->listWidget->clear();//每次打开文件清空列表
    ui->listWidget->insertItems(0,m_strlst);//把歌曲或者视频添加到播放列表
    foreach(QString const& arg,m_strlst){//遍历m_strlist中所有的 string
...
void Widget::setget_alltime(qint64 playtime)//获取视频总时长并设置到标签,滑条
{
    ...
    m_minute = (playtime - m_hour*3600)/60;
    m_second = playtime-m_hour*3600-m_minute*60;
    //qDebug()<<m_hour<<m_munete<<m_second;
    QString str = QString("%1:%2:%3").arg(m_hour).arg(m_minute).arg(m_second);
...
void Widget::on_btn_last_clicked()//上一个
{
...
void Widget::on_btn_next_clicked()//下一个
{
    m_pMediaPlayer->stop();//先暂停
